<section class="module-screen">
    @if ($can_manage_privacy)
        <div class="surface-card" id="privacy-activity-editor">
            <div class="row-between" style="margin-bottom:14px;">
                <div>
                    <div class="eyebrow">Create</div>
                    <div class="screen-title" style="font-size:26px;">New Processing Activity</div>
                </div>
            </div>
            ...
            <form class="upload-form" method="POST" action="{{ $create_route }}">
                ...
                <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                    ...
                    <div class="field">
                        ...
                        <input class="field-input" id="activity-lawful-basis" name="lawful_basis" placeholder="contract, legal obligation, legitimate interests" required>
                    </div>
                    <div class="field">
                    ...
                    <div class="field">
                        <label class="field-label" for="activity-owner">Owner actor</label>
            ...
            <tbody>
                @foreach ($activities as $activity)
                    <tr>
                        ...
                        <td>
                            @if ($activity['owner_assignment'] !== null)
                                <div>{{ $activity['owner_assignment']['display_name'] }}</div>
                                <div class="table-note">{{ $activity['owner_assignment']['kind'] }}</div>
                            @else
                                <span class="muted-note">No owner assigned</span>
                            @endif
                        </td>
                        <td>
                            <div>Flows: {{ $activity['linked_data_flow_ids'] !== '' ? $activity['linked_data_flow_ids'] : 'None' }}</div>
                            <div>Risks: {{ $activity['linked_risk_ids'] !== '' ? $activity['linked_risk_ids'] : 'None' }}</div>
                            <div>Policy: {{ $activity['linked_policy_id'] !== '' ? $activity['linked_policy_id'] : 'None' }}</div>
                            <div>Finding: {{ $activity['linked_finding_id'] !== '' ? $activity['linked_finding_id'] : 'None' }}</div>
                        </td>
                        <td>{{ $activity['review_due_on'] !== '' ? $activity['review_due_on'] : 'No review date' }}</td>
                        <td>
                            @forelse ($activity['artifacts'] as $artifact)
                            ...
                            @endforelse
                            @if ($can_manage_privacy)
                                <form class="upload-form" method="POST" action="{{ $activity['artifact_upload_route'] }}" enctype="multipart/form-data" style="margin-top:10px;">
                                    @csrf
                                    <input type="hidden" name="principal_id" value="{{ $query['principal_id'] }}">
                                    <input type="hidden" name="organization_id" value="{{ $query['organization_id'] }}">
                                    <input type="hidden" name="locale" value="{{ $query['locale'] }}">
                        ...
                        <td>
                            @if ($activity['transitions'] !== [])
                                <div class="action-cluster">
                                    @foreach ($activity['transitions'] as $transition)
                                        <form method="POST" action="{{ str_replace('__TRANSITION__', $transition, $activity['transition_route']) }}">
                                            @csrf
                                            <input type="hidden" name="principal_id" value="{{ $query['principal_id'] }}">
                                            <input type="hidden" name="organization_id" value="{{ $query['organization_id'] }}">
                                            <input type="hidden" name="locale" value="{{ $query['locale'] }}">
                            ...
                            @if ($can_manage_privacy)
                                <details style="margin-top:10px;">
                                    ...
                                    <form class="upload-form" method="POST" action="{{ $activity['update_route'] }}" style="margin-top:10px;">
                                        @csrf
                                        <input type="hidden" name="principal_id" value="{{ $query['principal_id'] }}">
                                        <input type="hidden" name="organization_id" value="{{ $query['organization_id'] }}">
                                        <input type="hidden" name="locale" value="{{ $query['locale'] }}">
                                        ...
                                        <div class="field">
                                            ...
                                            <input class="field-input" name="title" value="{{ $activity['title'] }}" required>
                                        </div>
                                        <div class="field">
                                            <label class="field-label">Lawful basis</label>
                                            <input class="field-input" name="lawful_basis" value="{{ $activity['lawful_basis'] }}" required>
                                        </div>
                                        <div class="field">
                                            <label class="field-label">Review due</label>
                                            <input class="field-input" name="review_due_on" type="date" value="{{ $activity['review_due_on'] }}">
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="scope_id">
                                                <option value="">Organization-wide</option>
                                                @foreach ($scope_options as $scope)
                                                    <option value="{{ $scope['id'] }}" @selected($activity['scope_id'] === $scope['id'])>{{ $scope['name'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="linked_data_flow_ids">
                                                <option value="">No linked data flow</option>
                                                @foreach ($data_flow_options as $flow)
                                                    <option value="{{ $flow['id'] }}" @selected($activity['linked_data_flow_ids'] === $flow['id'])>{{ $flow['label'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="linked_risk_ids">
                                                <option value="">No linked risk</option>
                                                @foreach ($risk_options as $risk)
                                                    <option value="{{ $risk['id'] }}" @selected($activity['linked_risk_ids'] === $risk['id'])>{{ $risk['label'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="linked_policy_id">
                                                <option value="">No linked policy</option>
                                                @foreach ($policy_options as $policy)
                                                    <option value="{{ $policy['id'] }}" @selected($activity['linked_policy_id'] === $policy['id'])>{{ $policy['label'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="linked_finding_id">
                                                <option value="">No linked finding</option>
                                                @foreach ($finding_options as $finding)
                                                    <option value="{{ $finding['id'] }}" @selected($activity['linked_finding_id'] === $finding['id'])>{{ $finding['label'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            ...
                                            <select class="field-select" name="owner_actor_id">
                                                ...
                                                @foreach ($owner_actor_options as $actor)
                                                    <option value="{{ $actor['id'] }}" @selected(($activity['owner_assignment']['id'] ?? null) === $actor['id'])>{{ $actor['label'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="field">
                                            <label class="field-label">Purpose</label>
                                            <input class="field-input" name="purpose" value="{{ $activity['purpose'] }}" required>
                                        </div>
                                        <div class="action-cluster">
