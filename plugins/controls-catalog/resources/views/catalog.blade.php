...
<section class="module-screen">
        ...
        <div class="surface-card" id="control-editor" hidden>
            <div class="row-between" style="margin-bottom:14px;">
                <div>
                    ...
                    <div class="entity-title" style="font-size:24px;">{{ __('New control') }}</div>
                </div>
            </div>
            ...
            <form class="upload-form" method="POST" action="{{ $create_route }}">
                ...
                <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                    <div class="field">
                        <label class="field-label" for="control-name">{{ __('Name') }}</label>
                        <input class="field-input" id="control-name" name="name" required>
                    </div>
                    <div class="field">
                        <label class="field-label" for="control-framework">{{ __('Framework') }}</label>
                    ...
                    <div class="field">
                        <label class="field-label" for="control-domain">{{ __('Domain') }}</label>
                        <input class="field-input" id="control-domain" name="domain" required>
                    </div>
                    <div class="field">
                        <label class="field-label" for="control-scope">{{ __('Scope') }}</label>
                    ...
                    <div class="field" style="grid-column:1 / -1;">
                        <label class="field-label" for="control-evidence">{{ __('Evidence summary') }}</label>
                        <input class="field-input" id="control-evidence" name="evidence" required>
                    </div>
                    <div class="field">
                        <label class="field-label" for="control-owner">{{ __('Initial owner actor') }}</label>
                ...
                <div class="action-cluster" style="margin-top:14px;">
                    <button class="button button-primary" type="submit" @disabled(! $hasFrameworks)>{{ __('Create control') }}</button>
                    @if (! $hasFrameworks)
                        <span class="muted-note">{{ __('Add a framework before adding controls.') }}</span>
                    @endif
                </div>
            </form>
        </div>
    ...
    <div class="overview-grid" style="grid-template-columns:repeat(6, minmax(0, 1fr));">
        <div class="metric-card"><div class="metric-label">{{ __('Controls') }}</div><div class="metric-value">{{ count($controls) }}</div></div>
        ...
        <div class="metric-card"><div class="metric-label">{{ __('Approved') }}</div><div class="metric-value">{{ collect($controls)->where('state', 'approved')->count() }}</div></div>
        <div class="metric-card"><div class="metric-label">{{ __('In Review') }}</div><div class="metric-value">{{ collect($controls)->where('state', 'review')->count() }}</div></div>
    </div>
    ...
    @if ($isDetailMode)
        <div class="table-card">
            <div class="surface-note" style="margin-bottom:16px;">
                {{ __('Control Detail keeps requirement mappings, ownership, evidence, workflow transitions, and control editing in one workspace. Use the control list to browse the catalog and open the control you want to work on.') }}
            </div>

            <div class="screen-header">
                <div>
                    <div class="eyebrow">{{ __('Control Detail') }}</div>
                ...
                <div class="action-cluster">
                    ...
                    <span class="pill {{ $ctrlStatePill }}">{{ __($selectedControl['state']) }}</span>
                </div>
            </div>

            <div class="overview-grid" style="grid-template-columns:repeat(4, minmax(0, 1fr));">
                <div class="metric-card">
                    <div class="metric-label">{{ __('Framework') }}</div>
                    <div class="metric-value">{{ $selectedControl['framework'] }}</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">{{ __('Domain') }}</div>
                ...
                <div class="metric-card">
                    <div class="metric-label">{{ __('Artifacts') }}</div>
            ...
            <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                <div class="surface-card" style="padding:16px;">
                    <div class="field-label">{{ __('Coverage') }}</div>
                    <div class="data-stack" style="margin-top:10px;">
                        @forelse ($selectedControl['requirements'] as $requirement)
                            <div class="data-item">
                                ...
                                <div class="table-note">{{ $requirement['framework_name'] }} · {{ __($requirement['coverage']) }}</div>
                                @if ($requirement['notes'] !== '')
                                    <div class="table-note">{{ $requirement['notes'] }}</div>
                                @endif
                            </div>
                        @empty
                            <span class="muted-note">{{ __('No requirements linked.') }}</span>
                        @endforelse
                    </div>

                    @if ($can_manage_controls)
                        <details style="margin-top:12px;">
                            <summary class="button button-secondary" style="display:inline-flex;">{{ __('Link requirement') }}</summary>
                            <form class="upload-form" method="POST" action="{{ $selectedControl['attach_requirement_route'] }}" style="margin-top:12px;">
                                ...
                                <select class="field-select" name="requirement_id" @if ($hasRequirements) required @endif>
                                    <option value="">{{ $hasRequirements ? __('Link a requirement') : __('Add a requirement first') }}</option>
                                ...
                                <input type="text" name="notes" placeholder="{{ __('Coverage note') }}">
                                <button class="button button-secondary" type="submit" @disabled(! $hasRequirements)>{{ __('Link requirement') }}</button>
                            </form>
                        </details>
                    @endif
                </div>
                ...
                <div class="surface-card" style="padding:16px;">
                    <div class="field-label">{{ __('Owners') }}</div>
                    <div class="data-stack" style="margin-top:10px;">
                ...
                <div class="surface-card" style="padding:16px;">
                    <div class="field-label">{{ __('Evidence') }}</div>
                    ...
                    <div class="data-stack" style="margin-top:12px;">
                        ...
                        @empty
                            <span class="muted-note">{{ __('No artifacts yet') }}</span>
                        @endforelse
                    </div>

                    @if ($can_manage_controls)
                        <details style="margin-top:12px;">
                            <summary class="button button-secondary" style="display:inline-flex;">{{ __('Attach evidence') }}</summary>
                            <form class="upload-form" method="POST" action="{{ $selectedControl['artifact_upload_route'] }}" enctype="multipart/form-data" style="margin-top:12px;">
                                ...
                                <button class="button button-secondary" type="submit">{{ __('Attach evidence') }}</button>
                            </form>
                        </details>
                    @endif
                </div>
            ...
            <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                <div class="surface-card" style="padding:16px;">
                    <div class="field-label">{{ __('Workflow') }}</div>
                    ...
                    @else
                        <div class="body-copy" style="margin-top:10px;">{{ __('View-only access') }}</div>
                    @endif
                </div>
                ...
                @if ($can_manage_controls)
                    <div class="surface-card" style="padding:16px;">
                        <details>
                            <summary class="button button-ghost" style="display:inline-flex; width:fit-content;">{{ __('Edit control details') }}</summary>
                            <form class="upload-form" method="POST" action="{{ $selectedControl['update_route'] }}" style="margin-top:14px;">
                                ...
                                <div class="field">
                                    <label class="field-label">{{ __('Domain') }}</label>
                                ...
                                <div class="field">
                                    <label class="field-label">{{ __('Evidence summary') }}</label>
                                ...
                                <div class="action-cluster">
                                    <button class="button button-secondary" type="submit" @disabled(! $hasFrameworks)>{{ __('Save changes') }}</button>
                                </div>
                            </form>
                        </details>
                    </div>
    ...
    @else
        <div class="surface-card">
            <div class="entity-title">{{ __('Control list') }}</div>
            <div class="table-note" style="margin-top:6px;">{{ __('This list stays focused on catalog browsing, framework context, owner summary, and Open. Use Control Detail to manage requirement mappings, evidence, workflow, and control maintenance.') }}</div>
        </div>
                    ...
                    @forelse ($controls as $control)
                        <tr>
                            ...
                            <td>
                                @if (($control['owner_assignments'] ?? []) !== [])
                                    ...
                                    @if (count($control['owner_assignments']) > 1)
                                        @php $extraOwners = count($control['owner_assignments']) - 1; @endphp
                            ...
                            <td>
                                ...
                                <span class="pill {{ $sCtrlPill }}">{{ __($control['state']) }}</span>
                            </td>
                            <td>
                                <div class="action-cluster">
                                    <a class="button button-secondary" href="{{ $control['open_url'] }}&{{ http_build_query(['context_label' => __('Controls'), 'context_back_url' => $controls_list_url]) }}">{{ __('Open') }}</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" style="text-align:center; padding:28px;">
                                <span class="muted-note">{{ __('No controls yet') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
