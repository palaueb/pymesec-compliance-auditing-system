...
<section class="module-screen">
    <div class="surface-note">
        {{ __('Type, criticality, and classification are business-managed catalog values from `Reference catalogs`. Asset lifecycle states such as `active`, `review`, or `retired` are system-controlled.') }}
    </div>

    {{-- ── Creation form (hidden, toggled via toolbar) ──────────────────────── --}}
    @if ($can_manage_assets)
        <div class="surface-card" id="asset-editor" hidden>
            <div class="eyebrow" style="margin-bottom:8px;">{{ __('New asset') }}</div>
            <form class="upload-form" method="POST" action="{{ $create_route }}">
                ...
                <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                    <div class="field">
                        <label class="field-label" for="asset-name">{{ __('Name') }}</label>
                        <input class="field-input" id="asset-name" name="name" required>
                    </div>
                    <div class="field">
                        <label class="field-label" for="asset-type">{{ __('Type') }}</label>
                    ...
                    <div class="field">
                        <label class="field-label" for="asset-criticality">{{ __('Criticality') }}</label>
                    ...
                    <div class="field">
                        <label class="field-label" for="asset-classification">{{ __('Classification') }}</label>
                        <select class="field-select" id="asset-classification" name="classification" required>
                            <option value="">{{ __('Choose a classification') }}</option>
                    ...
                    <div class="field">
                        <label class="field-label" for="asset-scope">{{ __('Scope') }}</label>
                    ...
                    <div class="field">
                        <label class="field-label" for="asset-owner-actor">{{ __('Initial owner actor') }}</label>
                        <select class="field-select" id="asset-owner-actor" name="owner_actor_id">
                            <option value="">{{ __('No actor owner') }}</option>
                ...
                <div class="action-cluster" style="margin-top:14px;">
                    <button class="button button-primary" type="submit">{{ __('Create asset') }}</button>
                </div>
            </form>
        </div>
    @endif
    ...
    @if (is_array($selected_asset))
        ...
        <div class="surface-card" style="padding:20px; display:grid; gap:18px;">
            <div class="surface-note">
                {{ __('Asset Detail keeps workflow, accountability, and governed asset changes inside one record workspace. The catalog list stays focused on browse, compare, and open.') }}
            </div>

            {{-- Header --}}
            <div class="screen-header" style="margin-bottom:0; padding-bottom:16px;">
                <div>
                    <div class="eyebrow">{{ __('Asset') }}</div>
            ...
            <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                ...
                <div class="surface-card" style="padding:14px; background:rgba(255,255,255,0.4);">
                    <div class="metric-label" style="margin-bottom:8px;">{{ __('Accountability') }}</div>
                    <div class="table-note" style="margin-bottom:10px;">{{ __('Owners: :count', ['count' => count($selected_asset['owner_assignments'] ?? [])]) }}</div>
                    ...
                    @else
                        <div class="table-note">{{ __('No owner assigned') }}</div>
                    @endif
                </div>

                {{-- Workflow --}}
                <div class="surface-card" style="padding:14px; background:rgba(255,255,255,0.4);">
                    <div class="metric-label" style="margin-bottom:8px;">{{ __('Governance actions') }}</div>
                    ...
                    @else
                        <div class="table-note">{{ __('View-only access') }}</div>
                    @endif
            ...
            @if ($can_manage_assets)
                ...
                <details>
                    <summary class="button button-ghost" style="display:inline-flex; width:fit-content;">{{ __('Edit asset details') }}</summary>
                    <form class="upload-form" method="POST" action="{{ $selected_asset['update_route'] }}" style="margin-top:14px;">
                        ...
                        <div class="overview-grid" style="grid-template-columns:repeat(2, minmax(0, 1fr));">
                            ...
                            <div class="field">
                                <label class="field-label">{{ __('Criticality') }}</label>
                            ...
                            <div class="field">
                                <label class="field-label">{{ __('Classification') }}</label>
                            ...
                            <div class="field">
                                ...
                                <div class="table-note">{{ __('Selecting an actor adds another accountable owner instead of replacing the current set.') }}</div>
                            </div>
                        </div>
                        <div class="action-cluster" style="margin-top:14px;">
    ...
    @else
        ...
        {{-- ── Summary metrics ───────────────────────────────────────────────── --}}
        <div class="overview-grid" style="grid-template-columns:repeat(4, minmax(0, 1fr));">
            <div class="metric-card"><div class="metric-label">{{ __('Assets') }}</div><div class="metric-value">{{ count($assets) }}</div></div>
            <div class="metric-card"><div class="metric-label">{{ __('Active') }}</div>   <div class="metric-value" style="color:#166534;">{{ collect($assets)->where('state', 'active')->count() }}</div></div>
            <div class="metric-card"><div class="metric-label">{{ __('In review') }}</div><div class="metric-value" style="color:#92400e;">{{ collect($assets)->where('state', 'review')->count() }}</div></div>
            <div class="metric-card"><div class="metric-label">{{ __('Retired') }}</div>  <div class="metric-value" style="color:var(--muted);">{{ collect($assets)->where('state', 'retired')->count() }}</div></div>
        </div>

        <div class="surface-card" style="padding:14px;">
            <div class="row-between" style="gap:12px; align-items:flex-start;">
                <div>
                    <div class="entity-title">{{ __('Asset catalog list') }}</div>
                    <div class="table-note">{{ __('This list stays focused on browse, compare, and open. Ownership, workflow, and governed changes stay in Asset Detail.') }}</div>
                </div>
            </div>
        </div>
                ...
                <tbody>
                    @forelse ($assets as $asset)
                        <tr>
                            ...
                            <td>
                                @if (($asset['owner_assignments'] ?? []) !== [])
                                    ...
                                    @if (count($asset['owner_assignments']) > 1)
                                        <div class="table-note">+{{ count($asset['owner_assignments']) - 1 }} {{ __('additional owner(s)') }}</div>
                                    @else
                                        <div class="table-note">{{ $asset['owner_assignments'][0]['kind'] }}</div>
                                    @endif
                                @else
                                    <span class="table-note">{{ __('No owner') }}</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <a class="button button-secondary" href="{{ $asset['open_url'] }}&{{ http_build_query(['context_label' => 'Assets', 'context_back_url' => $assets_list_url]) }}">{{ __('Open') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" style="text-align:center; padding:28px;">
                                <span class="muted-note">{{ __('No assets yet.') }}</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
