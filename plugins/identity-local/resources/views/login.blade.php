...
<body>
    <main class="login-card">
        ...
        <p>Enter your work email and we will send a secure one-time link. Local admins and organization users use the same flow.</p>

        <form method="POST" action="{{ route('plugin.identity-local.auth.request') }}">
            @csrf
            <div class="field">
                <label class="field-label" for="login-email">Work email</label>
                <input class="field-input" id="login-email" name="email" type="email" required autofocus>
            </div>
            <button class="button button-primary" type="submit">Send sign-in link</button>
        </form>
